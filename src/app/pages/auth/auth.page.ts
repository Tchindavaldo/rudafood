...
export class AuthPage implements OnInit {
  ...
  constructor(
    ...
    private getUserServices: getUserByIdServices,
  ...
  connectUser() {
    if (this.email != '' && this.password != '') {
      ...
      this.requeteToAuth
        ...
        .then(async user => {
          const data = await this.getUserServices.getUserById(user.uid);
          console.log('user get  ', data);

          this.userStorage.set('photoUrl', user.photoURL);
          this.userStorage.set('user', { ...data, ...user });
  ...
  //   this.presentToast(
  //     'bottom',
  //     "cet email est lié à une authentification via Google veuillez cliquer sur l'icone de google en haut pour vous connecter"
  //   );
  // }
