...
export class ManagePassedOrderStatusPage implements OnInit, OnDestroy {
  orderData: any[] | null = null;
  selectedChip = 'pending';
  userOders!: Observable<any[]>; // Utilisation d'un Observable
  currentDate!: string;

  activeRoute: string = 'user-pending-order';
  showAnim = false;
  // Gestion des dates
  ...
  handleStoreOrder = (Date: Date, orders: any[]) => {
    ...
    // console.log('delivering', delivering);
  ...
  async fetchFastFoodOrders() {
    try {
      this.userOders = this.store.select(state => state.userOrder.orders);

      this.userOders.subscribe(orders => {
        // console.log('Changement détecté depuis commande.ts', orders);

        this.orderData = orders;
        if (orders !== null) {
          this.handleStoreOrder(this.selectedDate, this.orderData);
      ...
      if (this.orderData !== null) {
        ...
        await this.getUserOrdersService.getFastUserOrders();
        this.router.navigate(['tabs/tab2/status-order/user-pending-order-NoAnim']);
      }
    } catch (error) {
  ...
  private initializeDates() {
    ...
    console.log('Dates finales dans orderDates:', Array.from(this.orderDates));
    console.log('Dates finales dans orderDates:', this.nextDays);
    ...
    console.log('allPassPendingDates', this.PassPendingDays, 'taille:', this.PassPendingDays.length);
    console.log('allPassProcessingDates', this.PassProcessingDays, 'taille:', this.PassProcessingDays.length);
    console.log('allPassFinishedDates', this.PassFinishedDays, 'taille:', this.PassFinishedDays.length);
    // console.log(
    //   'Dates des commandes:',
    //   this.nextDays.map(d => ({
    //     day: d.day,
  ...
  selectDay(day: { fullDate: Date }) {
    ...
    this.handleStoreOrder(day.fullDate, this.orderData || []);
  }
