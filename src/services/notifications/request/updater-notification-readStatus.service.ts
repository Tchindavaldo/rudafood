...
export class markNotificationAsReadService {
  ...
  async markNotificationAsRead(notificationId: string, notificationCreatedAt: any) {
    ...
    try {
      ...
      if (error.message === 'Network Error') {
        const alreadyPending = this.pendingPayloads.some(p => p.notificationId === notificationId && p.userId === user.uid && p.notificationCreatedAt === notificationCreatedAt);
        if (!alreadyPending) {
